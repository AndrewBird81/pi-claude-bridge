...
- **Add: name the default account (`provider.defaultAccountName`)** — with `accounts` configured, the default subscription still registered as bare `claude-bridge` next to a named `claude-bridge-work`, which reads as though only one of them is an account. Setting `defaultAccountName` registers it as `claude-bridge-<name>` too, so a personal/work pair is symmetric in the model picker. Its config dir is unchanged (still `CLAUDE_CONFIG_DIR`, or Claude Code's default) — only the provider name moves. The name is part of every model id, so pi's `enabledModels` and any pinned default model need updating alongside it. Unset, nothing changes.

- **Fix: thinking replay was keyed to the default account** — the rebuild path only replayed a thinking signature when the message came from provider `claude-bridge` exactly, so history produced by any named account was silently stripped of its thinking blocks. It now matches the account the session is being rebuilt into, which both restores replay for named accounts and keeps a signature minted by one account out of another's session.

- **Fix: providers vanished from hosts that re-enumerate extensions (oppi model picker)** — registration bailed out entirely once `ACTIVE_STREAM_SIMPLE_KEY` was set, so only the first module load in a process registered anything. A host that loads the extension again to harvest its providers (oppi's `ExtensionProviderCatalog`, which unregisters and re-applies on every `GET /models`) got nothing back on the second pass and dropped the bridge's models from the picker. Registration now always runs; the first instance still owns `streamSimple`, so a subagent's later load re-registers against the parent's stream function instead of overwriting it — which is what the guard's comment already described.

- **Add: multiple Claude accounts (`provider.accounts`)** — each entry registers a second provider (`claude-bridge-<name>`) backed by its own `configDir`, so two subscriptions (e.g. work and personal) are selectable side by side in one pi session. The config dir selects the account's credentials too — Claude Code derives its credential-store entry from `CLAUDE_CONFIG_DIR`, passed per query via the SDK's `env` option (pinned in `tests/int-cc-contracts.mjs`). An account toggle rebuilds the shared session under the other account's dir, wiping the old file from the dir it actually lives in; a session held by another account fails the resume guard rather than being resumed, including from AskClaude, which stays on the default account. Per-account `plan`/`longContextExtraUsage`. With `accounts` unset, behavior is unchanged.
